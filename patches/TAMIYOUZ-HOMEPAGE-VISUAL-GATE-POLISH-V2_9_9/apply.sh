...
VERSION="V2.9.9"
PATCH="TAMIYOUZ-HOMEPAGE-VISUAL-GATE-POLISH-V2_9_9"
ROOT="${WP_ROOT:-$PWD}"
...
for a in "نبني نموًا رقميًا" "لسنا مجرد وكالة رقمية" "حلول متكاملة" "من الفكرة إلى نظام" "الذكاء الاصطناعي ليس إضافة" "أعمال رقمية" "رحلة واضحة" "لديك فكرة أو تحدٍ"; do grep -Fq "$a" "$TPL"; done
grep -q "$PATCH" "$TPL"
grep -q "$PATCH" "$CSS"
grep -q "tamiyouz-template-fail-open-v2-9-9" "$TPL"
echo "TITLE=$TITLE"
